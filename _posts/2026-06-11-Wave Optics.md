...
---
<div style="border: 1px solid #ddd; border-radius: 8px; padding: 20px; background-color: #f9f9f9; text-align: center; font-family: sans-serif;">
    ...
    <canvas id="oscillatorCanvas" width="400" height="300" style="background-color: white; border: 1px solid #ccc; border-radius: 4px;"></canvas>
</div>
...
<script>
    ...
    function draw() {
        ...
        // 변위 y 계산: y(t) = A * cos(w * t)
        const A = 60; // 진폭
        const y = A * Math.cos(w * time);
        
        // 그래프를 위해 변위 기록
        history.unshift(y);
        if (history.length > canvas.width / 2) history.pop();

        // 1. 용수철 그리기
        ctx.beginPath();
        ctx.moveTo(100, 0);
        ctx.lineTo(100, 100 + y);
        ctx.lineWidth = k * 1.5; // k가 크면 굵어짐
        ctx.strokeStyle = '#888';
        ctx.stroke();

        // 2. 물체 그리기
        const boxSize = 30 + (m * 5); // m이 크면 커짐
        ...
        for (let i = 0; i < history.length; i++) {
            ...
            const graphY = 100 + boxSize/2 + history[i];
